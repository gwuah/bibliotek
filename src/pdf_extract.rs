...
pub async fn extract_metadata_from_bytes(data: &[u8]) -> Result<PdfMetadata> {
    ...
    let path = temp_dir.join(format!("bibliotek_pdf_extract_{}.pdf", timestamp));

    let mut file = File::create(&path)
        .await
        .context("Failed to create temporary file for PDF")?;
    ...
    let result = extract_metadata_from_path(&path);
