class BibliotekApp {
  ...
  renderMetadata() {
    ...
    authorsList.innerHTML = this.metadata.authors
      .map(
        (authorAgg) =>
    ...
    tagsList.innerHTML = this.metadata.tags
      .map(
        (tagAgg) =>
    ...
    ratingsList.innerHTML = this.metadata.ratings
      .map(
        (ratingAgg) =>
  ...
  createBookCard(book) {
    ...
    return `
            <div class="book-card">
                ...
                <button class="download-btn" onclick="window.open('${
                  book.download_url
                }', '_blank')">
                    Download
                </button>
  ...
  renderPagination() {
    ...
    if (this.currentPage > 1) {
      paginationHTML += `<button class="page-btn" onclick="app.goToPage(${
        this.currentPage - 1
      })">‹ Previous</button>`;
    }
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        await this.uploadFile(file, (progress) => {
          const overallProgress =
            ((i + progress.overallProgress / 100) / files.length) * 100;
          progressFill.style.width = `${overallProgress}%`;
          statusMessage.textContent = `Uploading ${file.name}... ${Math.round(
  ...
  async uploadFile(file, onProgress) {
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      await this.uploadChunk(uploadId, chunk, partNumber, (loaded) => {
